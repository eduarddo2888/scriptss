...
namespace DevionGames{
	...
	public static class EditorTools {
        ...
        public static string SearchField(string search,bool focus = false, params GUILayoutOption[] options)
        {
            ...
            if (Event.current.type == EventType.MouseUp && buttonRect.Contains(Event.current.mousePosition) || before == "Search..." && 
                GUI.GetNameOfFocusedControl() == "SearchTextFieldFocus")
            {
                before = "";
                GUI.changed = true;
            ...
            GUI.Button(buttonRect, GUIContent.none, (after != "" && after != "Search...") ? "ToolbarSeachCancelButton" : 
        ...
        public static bool Titlebar(string hash, GUIContent content, object target, GenericMenu menu)
        {
            ...
            int controlID = EditorGUIUtility.GetControlID(FocusType.Passive);
             ...
             if (Event.current.type == EventType.Repaint)
             {
                 ...
                 Styles.inspectorBigTitle.Draw(new Rect(position.x, position.y, position.width + 4f, position.height), GUIContent.none, 
                     controlID, false);
                 GUI.color = color;
             }

             Rect rect = new Rect(position.x + (float)Styles.inspectorTitle.padding.left, position.y + (float)Styles.inspectorTitle.padding.
                 top, 16f, 16f);
             ...
             if (menu != null && eventType == EventType.MouseDown && Event.current.button == 1 && position.Contains(Event.current.
                 mousePosition))
             {
                 menu.ShowAsContext();
             }
        ...
        private static bool MainActionKeyForControl(Event evt, int controlId)
        {
            ...
            return (evt.type != EventType.KeyDown || evt.keyCode != KeyCode.Space && evt.keyCode != KeyCode.Return && evt.keyCode != 
                KeyCode.KeypadEnter ? false : !flag);
        }
        ...
        public static float PropertyElementField(SerializedProperty arrayProperty, int index, bool drawScript = true) {
            ...
            else
            {
                ...
                {
                    if (element.propertyPath.Contains(propertyPath + ".Array.data[" + index + "]") && !element.propertyPath.Replace
                        (propertyPath + ".Array.data[" + index + "].", "").Contains("."))
                    {
                        height += EditorGUIUtility.standardVerticalSpacing;
                        EditorGUILayout.PropertyField(element, true);
                        height += EditorGUI.GetPropertyHeight(element, true);
        ...
        public static bool HasCustomPropertyDrawer(Type type)
        {
            ...
            foreach (Type typesDerivedFrom in TypeCache.GetTypesDerivedFrom<GUIDrawer>())
            {
                ...
                for (int i = 0; i < (int)customAttributes.Length; i++)
                {
                    ...
                    FieldInfo field = customPropertyDrawer.GetType().GetField("m_Type", BindingFlags.Public | BindingFlags.NonPublic | 
        ...
        public static object DrawFields(Rect rect, object obj)
        {
            ...
            FieldInfo[] fields = type.GetSerializedFields().Where(x => !x.HasAttribute(typeof(HideInInspector))).GroupBy(x => x.Name).
        ...
        public static object DrawField(Rect rect, GUIContent label, object obj, object value, FieldInfo field, params GUILayoutOption[]
            options)
        {
        ...
        public static float CalcHeight(GUIContent label, object obj, object value, FieldInfo field)
        {
            ...
            if (type == typeof(string))
            {
                ...
                {
                    TextAreaAttribute attribute = field.GetCustomAttribute<TextAreaAttribute>();
                    return attribute.minLines * EditorGUIUtility.singleLineHeight + EditorGUIUtility.singleLineHeight + EditorGUIUtility.
            ...
            foreach (Type typesDerivedFrom in TypeCache.GetTypesDerivedFrom<CustomDrawer>())
            {
                ...
                for (int i = 0; i < customAttributes.Length; i++)
                {
                    ...
                    if (customDrawer.UseForChildren)
                    {
                        ...
                        {
                            if ((!EditorTools.m_DrawerTypeForType.ContainsKey(type) ? true : !customDrawer.Type.IsAssignableFrom(EditorTools.
                                m_DrawerTypeForType[type].type)))
                            {
